...
addOrder unitTag ability obs =
  obs
    ...
    . Proto.units
    . traverse
    . filtered (\unit -> unit ^. #tag == unitTag)
    . #orders
    %~ (order :) -- TODO: append order to the end?
 where
...
getUnit obs utag =
  headMay $
    runC $
      obsUnitsC obs .| filterC (\u -> u ^. #tag == utag) .| filterC (\u -> u ^. #tag == utag)
...
findExpandPosInCluster grid heightMap cluster =
  bfsRes $
    gridBfs
      grid
      (tilePos . view #pos . head $ cluster)
...
findNexus :: Observation -> Maybe Proto.Unit
findNexus obs = headMay $ runC $ unitsSelf obs .| unitTypeC ProtossNexus

getNexus :: Observation -> Proto.Unit
getNexus = fromJust . findNexus
