...
enemyInRange u enemies =
  headMay $ filter (\e -> distSquaredF e u <= 6 * 6) enemies -- TODO: magic number Stalker attack range of 6
...
nearestEnemy u = minimumByMay (comparing (distSquaredF u))

nearestTarget :: Unit -> [Point2D] -> Maybe Point2D
nearestTarget u = minimumByMay (comparing (distSquaredF u))
