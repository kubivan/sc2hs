...
def cpp_enum_to_haskell(cpp_enum):
    enum_contents = re.search(r'enum\s+class\s+UNIT_TYPEID\s*{([\s\S]*?)};', cpp_enum).group(1)
    ...
    for line in enum_lines:
        ...
        if '=' in line and '//' in line:
            ...
            comment = ' '.join(parts[1].split('//')[1:]).strip()
    ...
    haskell_declaration = "data UnitTypeId =\n" + '\n'.join([line for _, _, line in haskell_lines]) + "\n    deriving (Show, Eq, Enum)"

    from_enum_function = "\n\nfromEnum :: UnitTypeId -> Int\nfromEnum = \\case\n" + '\n'.join([f"    {enum} -> {value}" for enum, value, _ in haskell_lines]) + "\n"

    to_int_function = "\n\ntoInt :: UnitTypeId -> Int\ntoInt = fromEnum\n"

    haskell_code = haskell_declaration + from_enum_function + to_int_function

    return haskell_code


haskell_enum = cpp_enum_to_haskell(cpp_enum)
print(haskell_enum)