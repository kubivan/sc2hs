...
networkParser includeClientPort =
 ...
 where
  clientOverride
    | includeClientPort =
        optional (option auto (long "port-client" <> metavar "PORT" <> help "Override client port"))
...
opponentParser =
  OpponentOverrides
    <$> optional
      ( option
          (eitherReader parseOpponentType)
          (long "opponent-type" <> metavar "ai|bot" <> help "Override opponent type")
      )
    <*> optional
      (option (eitherReader parseRace) (long "ai-race" <> metavar "RACE" <> help "Override AI race"))
    <*> optional
      ( option
          (eitherReader parseDifficulty)
          (long "ai-difficulty" <> metavar "DIFFICULTY" <> help "Override AI difficulty")
      )
    <*> optional
      ( option
          (eitherReader parseBuildOverride)
          (long "ai-build" <> metavar "BUILD|none" <> help "Override AI build or pass 'none' to remove")
      )
