...
stepTowardsTechGoal goal = do
  ...
  unless (null pathTobuild) $ do
    ds <- agentGet
    obs <- agentObs
    simLoop <- show <$> agentSimulationLoop
              ...
              closestMineralTo nexus =
                fromJust $
                  runConduitPure $
                    obsUnitsC obs
                      .| filterC isMineral
                      .| findC (\m -> distManhattan (toPoint2D $ m ^. #pos) (toPoint2D $ nexus ^. #pos) <= 12)
          command
...
instance Agent BotAgent where
  ...
  makeAgent _ playerId gi gameDataResp obs0 = do
    let heightMap = gridFromImage $ gi ^. #startRaw . #terrainHeight
        ...
        expands = sortOn (distSquared (toPoint2D nexusPos) . fromTuple) $ findExpands obsRaw grid heightMap
        enemyStart = tilePos $ toPoint3D $ enemyBaseLocation gi obsRaw

        playerInfos = gi ^. #playerInfo
        playerGameInfo = head $ filter (\gi -> gi ^. #playerId == playerId) playerInfos

        si = StaticInfo gi playerGameInfo unitTraits heightMap expands (tilePos nexusPos) enemyStart Nothing 0
  ...
  agentStep (BotAgent phase si ds env) obs abilities = do
    ...
    let errors = obs ^. #actionErrors
        simLoop = fromIntegral (obs ^. #observation . #gameLoop)
        si' = (maybe si (\result -> si{siAsyncStaticInfo = Just result}) asyncResult){siSimulationLoop = simLoop}
