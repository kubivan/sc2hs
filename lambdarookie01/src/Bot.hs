...
issueSelfCommandReserveAware ::
  (HasObs d, HasReservedCost d) => AbilityId -> [Units.Unit] -> StepMonad d ()
      ...
      nexuses =
        runC $
          units
            ...
            .| filterC (\n -> (n ^. #buildProgress) == 1 && null (n ^. #orders))
            .| unitIdleC
      ...
      mineralField =
        runConduitPure $
          probes
            ...
            .| mapC
              ( \harvester -> head $ filter (\o -> HARVESTGATHERPROBE == toEnum' (o ^. #abilityId)) (harvester ^. #orders)
              )
    ...
    then do
      ...
      if not . null $ nexusesUnder
        then do
          ...
          command
            [ UnitCommand HARVESTGATHERPROBE [idle] (fromJust $ mineralField <|> closestMineral idle)
            | idle <- idleWorkers
            ]
...
debugSquads = do
  ...
  debugTexts
    [ ( "s "
          ++ show (u ^. #tag)
          ++ " at "
          ++ show (HashMap.lookup (tilePos (u ^. #pos)) regionLookupMap)
          ++ " "
          ++ show (u ^. #facing)
      , u ^. #pos
      )
...
formatActionError err =
 ...
 where
  ...
  abilityName =
    if rawAbility == 0 then "UnknownAbility" else show (toEnum' (fromIntegral rawAbility) :: AbilityId)
...
instance Agent BotAgent where
  makeAgent ::
    BotAgent ->
    Word32 ->
    Proto.ResponseGameInfo ->
    Proto.ResponseData ->
    Proto.ResponseObservation ->
    IO BotAgent
