...
data BuildStructurePhase
  ...
  | -- builder, unit, startFrame, distance
    BSAccepted UnitTag Target Word32 Word32
...
cleanupIntent (PBuildStructure uid phase) =
  case phase of
    ...
    BSAccepted _ target _ _ -> releaseLandTarget uid target
...
traceBuilderOrders iid phase =
  case phase of
    ...
    BSAccepted builder _ _ _ -> logOrders builder
...
buildPhaseTimedOut :: Word32 -> Word32 -> Word32 -> Bool
buildPhaseTimedOut now startedAt distance =
  now >= startedAt
    -- TODO: naive penalty add some time to reach the target
    && now - startedAt > buildStartTimeoutFrames + distance * 5
