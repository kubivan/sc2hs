...
agentUpdateDsArmy = do
  ...
  let obsArmyUnits = unitsSelf obs .| filterC isArmyUnit .| filterC (\u -> u ^. #buildProgress == 1)
      ...
      freeArmyUnitTags =
        runC $
          obsArmyUnits .| mapC (view #tag) .| filterC (\utag -> not $ utag `Set.member` squadedUnitTags)
      ...
      army' =
        (dsArmy ds)
          { armyUnitsPos = Set.fromList obsArmyUnitsPoss
          , armyUnits = armyHashMap
          , armySquads = squadsFull ++ refilledSquads ++ newSquads
          }
...
randomArmyFiddling = do
  ...
  mapM_
    ( \u ->
        let unitData =
              HashMap.lookupDefault (ArmyUnitData Set.empty Set.empty) (u ^. #tag) (armyUnitsData . dsArmy $ ds)
...
randCmd2 grid udata u = do
  ...
  case enemyInRange u enemies of
    Just e -> command [Actions.UnitCommand ATTACKATTACK [u] e] -- Attack the enemy
    Nothing ->
      if not . null . view #orders $ u
        then return ()
          ...
          let army = dsArmy ds
              ...
              unvisitedNeighbors =
                filter (`Set.notMember` allUnitPos) $
                  filter (\p -> p `Set.notMember` auVisitedTiles udata) (neighbors upos grid)
 ...
 where
  ...
  nearest p = minimumBy (compare `on` distSquaredI p) --TODO: extract to utils
    ...
    Set.foldl'
      ( \score edgeTile ->
          let distToEdge :: TilePos -> Float
              distToEdge = fromIntegral . distSquaredI (edgeTile :: TilePos)
